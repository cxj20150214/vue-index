      ...
      <div class="adv_4" ref="putcontent">
        ...
        >
          ...
          <p class="hotline">
            ...
            <a :href="'tel:'+this.hotline">{{hotline}}</a>
          </p>
...
export default {
  ...
  data: function() {
    ...
    return {
      ...
      rules: {
        ...
        driving_age: [
          { required: true, message: "驾龄为必填", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    handleChange1(value) {
      ...
      this.$axios
        .get("/api/wechat/company/info", {
          id: value
        })
        .then(res => {
          console.log(res);
          this.hotline = res.data.kf_tel;
        });
    },
    getCity() {
      this.$axios.get("/api/wechat/region/list").then(res => {
        this.data = res.data;
      });
    },
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$axios.post("/api/wechat/driver/signup", data).then(res => {
            if (res.code == 200) {
              alert("提交成功");
            }
            if (res.code == 422) {
