<template>
  <div class="reward">
    <div class="rew_info">
      ...
      <p class="rew_price">
        ￥
        <input @change="search($event)" type="number" v-model="money" min="1" />
      </p>
...
export default {
  ...
  methods: {
    getorder() {
      let thisopenid = this.$route.query.openid;
      this.$axios.post("/api/shop/info", { openid: thisopenid }).then(
        res => {
          if (res.code === 200) {
